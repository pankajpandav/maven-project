pipeline {
  ...
  stages
  {
    ...
    stage('Deploy to Production')
    {
      steps
      {
          slackSend (channel: "#Builds", color: '#4286f4', message: "Deploy Approval: '${env.JOB_NAME} [${env.BUILD_NUMBER}]'", attachments: "[ { "text": "", "fallback": "You are unable to decide", "callback_id": "env.APPROVE_PROD", "color": "#3AA3E3", "attachment_type": "default", "actions": [ { "name": "approval", "text": "Approve", "type": "button", "value": "YES" }, { "name": "approval", "text": "Decline", "type": "button", "value": "NO" } ] } ]"  )
            script
            {
              try
              {
